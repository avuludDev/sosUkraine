...
module.exports = {
	...
	compileBundle ( files, bundle, done ) {
		...
		let critical = this.config.build.critical
		let criticalArray = []
		if(critical.length > 0){
			...
			this.gulp.src( criticalArray, options )
				.pipe( this.plumber() )
				.pipe( this.addGlobalHelper() )
				.pipe( this.compile() )
				.pipe( this.concat( 'critical' ) )
				.pipe( cssMinify() )
				.pipe( this.postcss( 'critical' ) )
				.pipe( this.destCritical( this.isDev ? options.sourcemaps : false ) )
				.pipe( this.cssnano() )
				.pipe( this.rename() )
